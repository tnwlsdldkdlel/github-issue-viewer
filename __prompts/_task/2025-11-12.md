...
### 요청 내용
수정할 때마다 캐시가 쌓이는 문제 해결

### 변경 사항
- `package.json`의 `dev` 스크립트 수정: 개발 서버 실행 시 자동으로 `.next/cache` 삭제
- `clean:cache` 스크립트 추가: 캐시만 삭제
- `clean:all` 스크립트 추가: `.next`와 `node_modules/.cache` 모두 삭제

### 파일
- `package.json`: 캐시 자동 삭제 스크립트 추가

## 성능 측정 재실행

### 요청 내용
우선순위 3 (가상 스크롤) 성능 측정 재실행 및 결과 업데이트

### 변경 사항
- Lighthouse 측정 재실행 (초기 로드, 500개 이슈)
- Chrome DevTools Performance 측정 재실행 (초기 로드, 500개 이슈)
- 측정 결과를 `virtual-scroll-template.md`에 업데이트
- 512개 이슈 로드 성공 확인 (가상 스크롤 컨테이너 내부 스크롤 트리거 정상 작동)

### 주요 결과
- DOM 노드 수: 7,551개 → 244개 (-96.8% 감소)
- 이슈 아이템 수: 522개 → 14개 (-97.3% 감소)
- 메모리 사용량: 55.88 MB → 53.01 MB (-5.1% 감소)

### 파일
- `docs/performance/virtual-scroll-template.md`: 성능 측정 결과 업데이트
